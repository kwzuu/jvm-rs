...
impl<'a> Runtime<'a> {
    pub fn new(main_class_name: &'a String) -> Result<Runtime, std::io::Error> {
        ...
        runtime.loaded.insert(main_class_name, runtime.main_class.clone());
        Ok(runtime)
    }
    ...
    pub fn load(&'a mut self, name: &'a String) -> Result<Rc<Class>, std::io::Error> {
        ...
        let cls = Rc::new(Class::from_classfile(ClassReader::new(name)?.read_classfile()));
    ...
    pub fn find_method(&'a mut self, cls: &'a String, name: &'a String, descriptor: &'a String) -> Result<Rc<Method>, ()> {
        let loaded = self.load(cls);
        match loaded {
    ...
    pub fn run_main(self: Rc<Self>) {
        let mut main_method = self.main_class.get_method(
            ...
            &"()I".to_string()
        ).unwrap_or_else(|()| self.main_class.get_method(
            &"main".to_string(),
            //&"([Ljava/lang/String;)V".to_string()
            &"()L".to_string()
        ...
        dbg!(main_method.exec(
            self.clone(),
            self.clone().main_class.clone(),
            frame,
        ).unwrap().int());
    }
}